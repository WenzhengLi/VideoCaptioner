# P02 Classify v003 — 内容来源、知识属性与相关性标注
...
2. `speaker_N` 是音频声纹聚类，表示同一说话人簇而不是 `unknown`。通读该簇的多个上下文样本，
   再判断其是讲师讲解、学员提问或其他角色；证据不足时可保留 `unknown`，但不得改写原始 speaker 聚类。
3. `speaker=teacher_a|teacher_b` 通常是 `instructor_explanation`；只有明确引用聊天原话时才是
   `actual_chat` 或 `quoted_statement`。
4. `content_type=board_ocr` 默认 `source_role=board`。OCR 出现聊天句子并不自动证明说话人身份。
5. “对方一定在想什么”“这是测试”等讲师判断标为 `instructor_claim`，不能标 observation。
6. 无法确认时使用 `unknown/uncertain`，不要为了填满分类强行判断。
