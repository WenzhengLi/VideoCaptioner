...
3. `course_chain` 七个节点逐项填写，课程未讲的部分明确留空；
4. 至少三个真正不同的回答方向，每方向至少三条自然表达；
5. 每方向写明目的、适用信号、不适用情境和表达强度；
6. 积极、顾虑、转移、降投四个分支各至少两条可直接使用的下一句；
7. 召回问法包含原句近义表达和上下文追问，不能只堆关键词；
8. 所有标签都有独立标签文件，并与 OB 建立双向链接；
9. frontmatter 计数与正文实际数量一致；
10. 课程事实与候选补足边界清楚，且状态保持待用户确认。
