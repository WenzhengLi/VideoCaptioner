...
十五课已完成程序验收。当前进入人工忠实度抽检和 C016–C020 最终扩展；Dify 在线部署和检索
验收完成前，不宣称已经在线入库。

## 五、当前立即动作

1. 抽检十五课中的修订、人工复核和拒绝案例；
2. 保持 C006/C008 的人工复核状态，不强行发布；
3. 保持 C014/C015 的拒绝状态，不通过人工改写绕过发布闸门；
4. 使用同一 `mimo-method-v002` 扩展 C016–C020；
5. 使用 `data/dify/afeng-release-v002.4/` 做十五课离线发布验收；
6. Dify 部署和检索验收通过前，不宣称已经在线入库。
