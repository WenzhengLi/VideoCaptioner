...
每个 5 课或 10 课迭代组使用稳定的 `WaveId`，例如 `C006-C010`。所有等待脚本都接受
`-WaveId`、`-StartCourse` 和 `-EndCourse`；完成标记形如
`cursor-p04-knowledge-v002-C006-C010-complete.json`。下游阶段只读取同一波次的前置标记，
因此不会把 C001-C005 的完成状态误当成 C006-C010 已完成。

P01 守护脚本根据 `BatchId` 和 `RunVersion` 计算归档运行号，并等待对应逐课 transcript 与原始
QA。P02–P06 可以同时预启动，但只会在同波次前一阶段状态为 `complete` 后继续。任一阶段产生
`needs_review` 或 `failed` 标记时，下游立即停止并保留失败清单，供补跑后继续。
