...
### Flow G：Tidy/知识库存储和问答

- 当前已有 Tidy 兼容 Markdown + SQLite FTS，必须保留。中文检索已有 n-gram + IDF 回退，不能退化。
- 检查仓库和本机是否存在用户所指具体 Tidy 产品/API 的明确证据。若能确定产品，增加正式 adapter 和
  可重复导入命令；若无法确定，不要冒充已经接入某个外部产品，要完成通用 JSONL/Markdown 导出、稳定
  provider 接口和清晰配置说明。
...
20 课回归且全量使用；Tidy 兼容存储、检索和多方案回答真实可运行；最终报告和文档完成；测试通过；
