...
### 1. Document Map 键与 canonical ID 不匹配 → 实际 Dify 同步将创建 36 个重复文档

| 项 | 值 |
|---|---|
| **文件** | `src/course_video_analyzer/knowledge/dify_sync.py:294-296` |
| **严重性** | **HIGH**（阻塞 TASK-015/016 的 Dify 实际同步） |
| **TASK-014 影响** | 无 — dry-run 预期"36 create"是正确的 |

**现状**:

`data/dify/document-map.json` 的 36 个条目全部以非 canonical 键存储：

| 键格式 | 示例 | 数量 |
|---|---|---|
| SHA-256 哈希 | `31c71330391aa7b9...` | ~2 |
| 模型自写 | `afeng-method-C016-001`、`KN-C009-M001` | ~30 |
| 混合 | `AFENG-METHOD-C020-002` | ~4 |
| **Canonical** | `AFENG-C{NNN}-CASE-C{NNN}-{NNN}` | **0** |

当 TASK-015/016 对 v002.6 bundle 执行真实 `sync_markdown_dir` 时：

1. `_metadata_from_markdown` 计算 canonical ID → `AFENG-C011-CASE-C011-002`
2. `docs.get("AFENG-C011-CASE-C011-002")` → `None`（map 中键是哈希 `31c7133...`）
3. 走 `create` 路径而非 `update`
4. **结果**: Dify 中出现 36 个重复文档，原 v002.5 文档成为孤立条目

**修复窗口**: TASK-014 完成后、TASK-015/016 执行前。修复方案二选一：

- **方案 A**: 一次性迁移脚本，重映射 document-map.json 键为 canonical
- **方案 B**: 在 `_metadata_from_markdown` 查 map 前增加回退查找（遍历 map 匹配 `course_id`+`case_id`）

**复现方式**:

```python
# 对 v002.5 目录执行 dry-run 即可观察到键不匹配
plan = plan_markdown_sync(
    Path("data/dify/afeng-release-v002.5/documents"),
    Path("data/dify/document-map.json")
)
# TASK-014 dry-run 预期: create=36, update=0, skip=0 ✅
# 实际 Dify 同步后果: 36 duplicates in Dify ❌
```

**document-map.json 实际条目示例**:

```json
{
  "31c71330391aa7b911fc7c6abf67c02aee061bd7d380f40c08470ded7da80d18": {
    "document_id": "fad02c5f-0de0-4ed7-aab4-ab410e9cb151",
    "metadata": {
      "knowledge_id": "31c71330391aa7b911fc7c6abf67c02aee061bd7d380f40c08470ded7da80d18",
      "course_id": "C011",
...
v002.6 sync 将计算 `AFENG-C011-CASE-C011-002` 作为键，与上条不匹配 → create。
...
| **⚠️ document map 迁移** | **TASK-015/016 前** | 迁移脚本 + dry-run 验证 update/skip 行为 |
| Dify API update 幂等性 | TASK-015/016 | 少量文档手动 update + re-index 测试 |
| existing map orphan 清理 | TASK-015/016 | 迁移后检查旧键条目是否需要清理 |
...
3. 唯一的高风险项（document-map.json 键不匹配）必须在 TASK-015/016 执行真实 Dify 同步前通过迁移脚本解决。
4. 未提交差异为文档收尾，建议提交。
