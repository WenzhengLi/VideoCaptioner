...
$p05Complete = Join-Path $batchDir "cursor-p05-${OutputVersion}$waveSuffix-complete.json"
$statusPath = Join-Path $batchDir "cursor-p06-${OutputVersion}$waveSuffix-status.jsonl"
$failurePath = Join-Path $batchDir "cursor-p06-${OutputVersion}$waveSuffix-failures.jsonl"
...
for ($ordinal = $StartCourse; $ordinal -le $EndCourse; $ordinal++) {
    ...
    foreach ($case in $cases) {
        ...
        $p05 = Join-Path $DataRoot "courses\$courseId\04_knowledge\P05-${OutputVersion}\$caseId.json"
        $inputDir = Join-Path $DataRoot "courses\$courseId\05_tidy\P06-input-${OutputVersion}"
        $jsonDir = Join-Path $DataRoot "courses\$courseId\05_tidy\P06-${OutputVersion}"
        $markdownDir = Join-Path $DataRoot "courses\$courseId\05_tidy\markdown-${OutputVersion}"
        ...
        $qaOutput = Join-Path $DataRoot "courses\$courseId\qa\P06-$caseId-${OutputVersion}-qa.json"
...
@{
    schema_version = "1.0"; stage = "P06-tidy"; prompt_version = "${OutputVersion}-p06"
    ...
    Join-Path $batchDir "cursor-p06-${OutputVersion}$waveSuffix-complete.json"
)
